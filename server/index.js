...
app.post('/create_survey', (req, res) => {
    ...
    db.query(survey_query, (err, result) => {
        ...
        for (let i = 0; i < questions.length; i++) {
            let question_query = `insert into Questions (SurveyID, QuestionType, Question) values (${sid}, ${questionType[i].typeValue === 'true' ? 1 : 0}, "${questions[i].question}")`;
            db.query(question_query, (err, result) => {
                if(err) {
                    console.log(err);
                }
                console.log(`inserting the ${i} question. qt: ${questionType[i].typeValue}`);
            })
        }
        res.send(result);
    });
